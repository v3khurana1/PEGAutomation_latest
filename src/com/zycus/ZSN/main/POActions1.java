...
public class POActions1 extends CommonFunctions
{

    public POActions1(WebDriver driver, Actions action, ExtentTest logger)
        throws Exception
    {
        super(driver, logger);
        ...
        selectItemCheckboxXpath = By.xpath("//*[@id='frmShipmentNoticeCreate']//table//tr[1]/td/input[contains(@id,'chkItemNo')]");
        this.driver = driver;
        this.action = action;
        this.logger = logger;
    }

    public boolean downloadPOasPDF()
        throws Exception
    {
        boolean result = false;
        try
        {
            click(optionBtnxpath);
            click(downloadBtnid);
            result = true;
    ...
    public boolean previewPOascXML()
        throws Exception
    {
        ...
        try
        {
            ...
            {
                action.sendKeys(new CharSequence[] {
                    Keys.ESCAPE
                }).build().perform();
    ...
    public boolean createShipmentNotice(String action, String carrier, String Shipment_No, String Shipped_Via, String Service_level)
        throws Exception
    {
        ...
        {   if(!findElement(optionBtnxpath).getAttribute("class").contains("open"))
            ...
            click(shipViaid);
            click(By.xpath((new StringBuilder("//ul/li/a[text()='")).append(Shipped_Via).append("']").toString()));
            click(clearShipNoid);
            selectDate("txtShipmentDate");
            Thread.sleep(3000);
            click(serviceLevelid);
            click(By.xpath((new StringBuilder("//ul/li/a[text()='")).append(Service_level).append("']").toString()));
            selectDate("txtShippingEdd");
            click(selectItemCheckboxXpath);
            String s;
