...
public class CreateTemplate extends iSource_CommonFunctions {
	...
	public boolean createTemplateFromStart(String eventDesc, String purchasingGrp, boolean isPublicEvent) {
		...
		try {
			createNewEvent(eventDesc, purchasingGrp, isPublicEvent);
			selectQuesItems();
			selectTCs();
	...
	private void createNewEvent(String eventDesc, String purchasingGrp, boolean isPublicEvent) {
		try {
			...
			if(driver.findElement(By.id("flexiFormTitleDiv"))!=null)
				enterFlexiFields();
			//Displayed sometimes
			...
			findElement(By.xpath(
					"//tr[td/label[@title='Is this a test event']]//select/option[@title='" + isTestEvent + "']"))
							.click();
			findElement(By.xpath("//tr[td/label[@title='Links']]//select/option[@title='" + links + "']"))
					.click();
			...
			waitUntilInvisibilityOfElement(By.xpath("//div[@id='LoaderForDiv' and contains(@style,'none')]"));
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
	...
	private void selectEventType(String eventType, boolean isPublicEvent) {
		try {
			...
			if(driver.findElement(By.id("flexiFormTitleDiv"))!=null)
				wait_untilVisible(driver.findElement(By.xpath("//div[@id='flexiFormTitleDiv' and not(contains(@style,'none'))]")));
