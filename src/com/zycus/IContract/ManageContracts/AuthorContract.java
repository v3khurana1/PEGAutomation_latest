...
public class AuthorContract extends eInvoice_CommonFunctions {
	...
	/*public boolean createContract() throws Exception {
		...
		try {
			...
			if (objCreateContract.getPgHead() != null){
				logger.log(LogStatus.INFO, "clicked on 'Create Contract' button");
				result = true;
			}
		} catch (Exception e) {
			e.printStackTrace();
			logger.log(LogStatus.INFO, "Not navigated to 'Create Contract' page");
			throw new Exception();
		}
		return result;
	}*/
	...
	public boolean filterByContractNum(String contractNumber) {
		boolean result = false;
		try {
			result = filterByText("Contract Number", contractNumber) ? true : false;
		} catch (Exception e) {
			e.printStackTrace();
		}
		return result;
	...
	/*public boolean viewContract(String contractNum){
		boolean result = false;
		try{
			findElement(By.xpath("//*[@id='authorContractListing-grid']/tbody//td[2]/span[@title='"+contractNum+"']")).click();
			waitUntilInvisibilityOfElement(processingLoader);
