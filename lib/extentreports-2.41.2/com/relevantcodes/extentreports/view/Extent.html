...
<html>
	<head>
		...
		<link href='${protocol}://fonts.googleapis.com/css?family=Source+Sans+Pro:400,600' rel='stylesheet' type='text/css'>
		 ...
		 <!-- <link href='extent.css' rel='stylesheet' type='text/css'> -->
	...
	<body class='extent default ${theme} hide-overflow'>
		...
		<nav>
			<div class='logo-container blue darken-2'>
				<a class='logo-content' href='#!'>
					<span>${report.configurationMap["productName"]}</span>
				</a>
				<a href='#' data-activates='slide-out' class='button-collapse hide-on-large-only'><i class='mdi-navigation-apps'></i></a>
			</div>
			<ul id='slide-out' class='side-nav fixed hide-on-med-and-down'>
				<li class='analysis waves-effect active'><a href='#!' class='test-summary-view' onclick="_updateCurrentStage(0)"><i class='mdi-action-dashboard'></i>${resourceBundle.getString("nav.menu.testDetails")}</a></li>
				<li class='analysis waves-effect'><a href='#!' class='test-view' onclick="_updateCurrentStage(1)"><i class='mdi-action-dashboard'></i>${resourceBundle.getString("nav.menu.testDetails")}</a></li>
				<#if report.categoryTestMap?? && (report.categoryTestMap?size != 0)>
					<li class='analysis waves-effect'><a href='#!' class='categories-view' onclick="_updateCurrentStage(2)"><i class='mdi-maps-local-offer'></i>${resourceBundle.getString("nav.menu.categories")}</a></li>
				</#if>
				<#if report.exceptionTestMap?? && (report.exceptionTestMap?size != 0)>
					<li class='analysis waves-effect'><a href='#!' class='exceptions-view' onclick="_updateCurrentStage(3)"><i class='mdi-action-bug-report'></i>${resourceBundle.getString("nav.menu.exceptions")}</a></li>
				</#if>
			...
			<span class='report-name'><#if report.configurationMap??>${report.configurationMap["reportName"]}</#if></span> 
			<span class='report-headline'><#if report.configurationMap??>${report.configurationMap["reportHeadline"]}</#if></span>
		</nav>
		<!-- /nav -->
		...
		<div class='container'>
			...
			-->
			<!--Summary -->
			<div id='test-summary-view' class='row'>
				<div class='col l4 m12 s12'>
					<div class='card-panel'>
						<span class='label info outline right'>${resourceBundle.getString("dashboard.panel.name.environment")}</span>
						<table>
								...
								<ul id='test-collection' class='test-collection'>
									<#list report.testList as extentTest>
										...
										<li class='collection-item test displayed active ${test.status} <#if test.nodeList?size != 0>hasChildren</#if>' extentid='${test.id?string}'>
											...
											<div class='test-body'>
												<div class='test-info'>
													<span title='${resourceBundle.getString("tests.test.info.testStartTime")}' alt='${resourceBundle.getString("tests.test.info.testStartTime")}' class='test-started-time label green lighten-1 text-white'>${test.startedTime?datetime?string(dateTimeFormat)}</span>
													<span title='${resourceBundle.getString("tests.test.info.testEndTime")}' alt='${resourceBundle.getString("tests.test.info.testEndTime")}' class='test-ended-time label red lighten-1 text-white'><#if test.endedTime??>${test.endedTime?datetime?string(dateTimeFormat)}</#if></span>
